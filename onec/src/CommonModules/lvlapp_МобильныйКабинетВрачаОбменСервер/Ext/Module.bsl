...
Функция ПолучитьРасписаниеВрача(Параметры) Экспорт 
	...
	ЗначенияДополнительныхПараметров =  ПолучитьЗначениеДополнительныхПараметровУслуг(СменныеЗадания.ВыгрузитьКолонку("УникальныйИдентификаторУслуги"));
	...
	Для Каждого СтрокаДанные из СменныеЗадания Цикл                            
		...
		Если ЗначенияДополнительныхПараметровСтроки <> Неопределено Тогда
			Для Каждого Элемент ИЗ ЗначенияДополнительныхПараметровСтроки Цикл
				ДополнительныеПараметры.Добавить(Новый Структура(Элемент.Ключ, Элемент.Значение));
			КонецЦикла;
		КонецЕсли;
...
Функция ПолучитьЗначениеДополнительныхПараметровУслуг(УникальныеИдентификаторыУслуги) Экспорт
	...
	Пока ВыборкаУникальныйИдентификаторУслуги.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			ЗначенияПараметров.Вставить(Выборка.ИмяПредопределенныхДанных, Выборка.ТипЗначения.ПривестиЗначение(Выборка.Значение));	
		КонецЦИкла;
